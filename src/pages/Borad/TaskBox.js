...
const TaskBox = ({ task, filterValue, fetchTasks }) => {
  ...
  const completedTasksCount = currentTask.checkList.filter(item => item.isDone).length;
  ...
  const formatDate = (dateString) => {
    ...
    const getOrdinalSuffix = (day) => (day > 3 && day < 21) ? "th" : ["th", "st", "nd", "rd"][Math.min(day % 10, 3)];
    return `${day}${getOrdinalSuffix(day)} ${month}`;
  };

  const isDueDatePassed = task.dueDate ? new Date(task.dueDate) < new Date() : false;
  ...
  return (
    <div className="task-box-container1">
      ...
      <div className="priority-header1">
        <div>
          <span className={`priority-label1 ${task.priority.replace(" ", "-").toLowerCase()}`} />
  ...
  <li onClick={handleShare}>Share</li>
  {
    userId === task.createdBy && ( <li className='delete-option1' onClick={openDeleteModal} > Delete</li> )
      ...
      <h4 className="task-title1">
        {task.taskName}
        {task.taskName.length > 48 && <span className="tooltip">{task.taskName}</span>}
      </h4>
          ...
          {currentTask.checkList.map((item) => (
            <div className="checklist-item1" key={item.checkListID}>
              <input
                type="checkbox"
                checked={item.isDone}
                onChange={(e) => handleCheckChange(item.checkListID, e.target.checked)}
              />
      ...
      <div className="task-footer1">
        ...
        <div className="task-status-container1">
          {task.statusOptions.map((status, index) => (
            <span className="task-status1" key={index} onClick={() => updateStatus(status)}>
              {status}
            </span>
          ))}
        </div>
      ...
      {isDeleteModalOpen && <DeleteModal onCloseDeleteModal={closeDeleteModal} taskID={task.taskID} />}
      {isUpdateModalOpen && <UpdateTaskModal onClose={closeUpdateModal} taskData={task} />}
    </div>
  );
};
